    ...
    r"({d})|\[(.+?)\]\((.+?)\)".format(
        d="|".join(
            [
                ...
                for i in [
                    PRE_DELIM, CODE_DELIM, STRIKE_DELIM, 
                    UNDERLINE_DELIM, ITALIC_DELIM, BOLD_DELIM, SPOILER_DELIM
                ]
            ]
        )
    )
        ...
        for line in lines:
            if line.startswith(BLOCKQUOTE_DELIM):
                if not in_blockquote:
                    tag = "blockquote expandable" if line.startswith(BLOCKQUOTE_EXPANDABLE_DELIM) else "blockquote"
                    prefix = BLOCKQUOTE_EXPANDABLE_DELIM if tag == "blockquote expandable" else BLOCKQUOTE_DELIM
                    # Replace prefix and open tag
                    line = OPENING_TAG.format(tag) + line[len(prefix):].strip()
                    in_blockquote = True
                    result.append(line)
                else:
                    # Continue blockquote
                    prefix = BLOCKQUOTE_EXPANDABLE_DELIM if line.startswith(BLOCKQUOTE_EXPANDABLE_DELIM) else BLOCKQUOTE_DELIM
...
".join(result)
    ...
    def parse(self, text: str):
        ...
        for match in list(re.finditer(MARKDOWN_RE, text)):
            ...
            if text_url:
                # Standard link or CustomMarkdown pseudo-link
                text = utils.replace_once(text, full, URL_MARKUP.format(url, text_url), start)
                continue

            tag_map = {
                BOLD_DELIM: "b", ITALIC_DELIM: "i", UNDERLINE_DELIM: "u",
                STRIKE_DELIM: "s", CODE_DELIM: "code", PRE_DELIM: "pre",
...
")[0]
                        ...
                        text = utils.replace_once(text, line_content, replacement, start)
                        continue
    ...
    def unparse(self, text: str, entities: list):
        ...
        for entity in (entities or []):
            ...
            if isinstance(entity, types.MessageEntityCustomEmoji):
                ...
                start_tag, end_tag = f"{PRE_DELIM}{lang}
", f"
{PRE_DELIM}"
                ...
                start_tag = BLOCKQUOTE_EXPANDABLE_DELIM + " " if getattr(entity, 'collapsed', False) else BLOCKQUOTE_DELIM + " "
                # Blockquotes are handled line-by-line in unparsing usually, 
                # but for simplicity we apply it to the start of the block
        ...
        # Sort by offset descending to maintain string integrity during insertion
